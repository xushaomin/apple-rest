...
public class MessageMarshallerUtilsTest {
    ...
    @Test
    public void testMarshallerRestRequest() throws Exception {
        RestRequest restRequest = mock(RestRequest.class);
        RestRequestContext msc = mock(RestRequestContext.class);
        when(restRequest.getRestRequestContext()).thenReturn(msc);
        HashMap<String, String> map = new HashMap<String, String>();
        map.put("key1", "key1Value");
        map.put("key2", "key2Value");
        map.put("key3", "key3Value");
        when(msc.getAllParams()).thenReturn(map);
        String message = MessageMarshallerUtils.getMessage(restRequest, MessageFormat.json);
        ...
        message = MessageMarshallerUtils.getMessage(restRequest, MessageFormat.xml);
        assertNotNull(message);
        assertTrue(message.indexOf("<") > -1);
        assertTrue(message.indexOf(">") > -1);
        assertTrue(message.indexOf("key1") > -1);
    ...
    @Test
    public void testMarshallerRestResponse() throws Exception {
