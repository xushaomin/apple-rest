...
import com.appleframework.rest.Interceptor;
import com.appleframework.rest.RestException;
import com.appleframework.rest.ThreadFerry;
import com.appleframework.rest.config.InterceptorHolder;
import com.appleframework.rest.config.RestEventListenerHodler;
...
public class AnnotationServletServiceRouterFactoryBean
        ...
        try {
            if (StringUtils.hasText(threadFerryClassName)) {
                ...
                if (!ClassUtils.isAssignable(ThreadFerry.class, threadFerryClass)) {
                    throw new RestException(threadFerryClassName + "没有实现"
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        //注册监听器
        ArrayList<RestEventListener> listeners = getListeners();
        if (listeners != null) {
            for (RestEventListener listener : listeners) {
    ...
    @SuppressWarnings("rawtypes")
	private ArrayList<RestEventListener> getListeners() {
        Map<String, RestEventListenerHodler> listenerMap = this.applicationContext.getBeansOfType(RestEventListenerHodler.class);
        if (listenerMap != null && listenerMap.size() > 0) {
            ArrayList<RestEventListener> restEventListeners = new ArrayList<RestEventListener>(listenerMap.size());

            //从Spring容器中获取Interceptor
            for (RestEventListenerHodler listenerHolder : listenerMap.values()) {
                restEventListeners.add(listenerHolder.getRestEventListener());
            }
